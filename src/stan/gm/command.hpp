...
namespace stan {
  ...
  namespace gm {
    ...
    void write_contour_comment_block(std::ostream& sample_stream,
				     const std::string& data_file,
				     const std::string& init_val,
				     const bool save_warmup,
				     const contour_info& contour) {
      std::stringstream ss;
	
      write_comment(sample_stream,"Contour Information Generated by Stan");
      write_comment(sample_stream);
      write_comment_property(sample_stream,"stan_version_major",stan::MAJOR_VERSION);
      write_comment_property(sample_stream,"stan_version_minor",stan::MINOR_VERSION);
      write_comment_property(sample_stream,"stan_version_patch",stan::PATCH_VERSION);
      ...
      write_comment_property(sample_stream,"init",init_val);
      write_comment_property(sample_stream,"save_warmup",save_warmup);
      write_comment(sample_stream,"contour info:");
      ss << " " << contour.idx0 << ": " << "(" << contour.min0 << ", " << contour.max0 << ")";
      write_comment_property(sample_stream,"  index0",ss.str());      
      ss.str("");
      ss << " " << contour.idx1 << ": " << "(" << contour.min1 << ", " << contour.max1 << ")";
      write_comment_property(sample_stream,"  index1",ss.str());
      write_comment_property(sample_stream,"  n",contour.n);
      
      ss.str("");
      ss << "(";
      for (size_t i = 0; i < contour.reals.size(); i++) {
	if (i > 0)
	  ss << ",";
	ss << contour.reals[i];
      }
      ss << ")";
      write_comment_property(sample_stream,"  default_reals",ss.str());
      ss.str("");
      ss << "(";
      for (size_t i = 0; i < contour.ints.size(); i++) {
	if (i > 0)
	  ss << ",";
	ss << contour.ints[i];
      }
      ss << ")";
      write_comment_property(sample_stream,"  default_ints",ss.str());
      write_comment(sample_stream);
    }
	...
	write_contour_comment_block(sample_stream,
				    data_file,
				    init_val,
				    save_warmup,
				    contour);
