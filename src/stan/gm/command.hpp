...
namespace stan {
  ...
  namespace gm {
    ...
    void print_nuts_help(std::string cmd) {
      ...
      print_help_option(&std::cout,
                        "debug","file",
                        "File into which unconstrained samples are written");
    ...
    void sample_from(Sampler& sampler,
		     ...
		     std::ostream* unconstrained_sample_file_stream,
      ...
      for (int m = 0; m < num_iterations; ++m) {
	    ...
	    model.write_unconstrained_csv(params_r,params_i,unconstrained_sample_file_stream);
          } else {
    ...
    int nuts_command(int argc, const char* argv[]) {
      ...
      std::string unconstrained_sample_file = "";
      command.val("debug",unconstrained_sample_file);
...
//           fprintf(stderr, "   %f  (last = %f)\n", lp, lastlp);
      ...
      std::fstream *unconstrained_sample_stream=0;
      if (unconstrained_sample_file != "")
	unconstrained_sample_stream = new std::fstream(unconstrained_sample_file.c_str(),
						       samples_append_mode);
      ...
      write_sample_comment_block(&sample_stream,
				 ...
				 epsilon, epsilon_pm, delta, gamma);
      write_sample_comment_block(unconstrained_sample_stream,
				 data_file, init_val, append_samples, 
				 save_warmup, random_seed, chain_id,
				 num_iterations, num_warmup, num_thin,
				 equal_step_sizes, leapfrog_steps, max_treedepth,
        ...
        if (!append_samples) {
          ...
          model.write_csv_header(sample_stream);
	  model.write_unconstrained_csv_header(unconstrained_sample_stream);
        }
        ...
        sample_from(nuts2_sampler,epsilon_adapt,refresh,
                    num_iterations,num_warmup,num_thin,save_warmup,
                    sample_stream,unconstrained_sample_stream,
      ...
      sample_stream.close();
      if (unconstrained_sample_stream != 0) {
	unconstrained_sample_stream->close();
	delete(unconstrained_sample_stream);
      }
